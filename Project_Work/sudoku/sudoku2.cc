...
bool fill_in( std::vector<std::vector<int> > & grid, int depth, int & count) {
  ...
  return fill_in(grid, depth+1, count);
}
...
int main(int argc, char * argv[]) {
  ...
  int recur_count = 0;
  bool found_solution = fill_in(grid,0, recur_count);
